...
class Eqrouter {
  ...
  /// Singleton instance of the router
  static Eqrouter get instance => _instance;

  /// Registration of all versions, routes and handlers
  Map<String, VersionedRoutes> versions = {};
  static final Eqrouter _instance = Eqrouter._internal();
  ...
  RouteMatchHandler? matchRoute({
    ...
    log('matchRoute versionRoutes: ${versionRoutes.routes}');
    ...
    for (final MapEntry(:key, :value) in versionRoutes.routes.entries) {
      if (value.methods[method] != null) {
        ...
        log('matchRoute routePattern: $routePattern');
        ...
        log('matchRoute pathSegments: $pathSegments');
        ...
        for (var i = 0; i < routePattern.length; i++) {
          log('matchRoute $i: ${routePattern[i]}');
  ...
  Future<dynamic> handleRequest({
    ...
    log('pathRequest: $pathRequest');
    log('pathRequest: $pathSegments');
    log('version: $version');
    log('method: $method');
    log('path: $path');
